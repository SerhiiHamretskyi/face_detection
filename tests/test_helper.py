...
def test_no_face_in_reference_photo():
    with mock.patch('os.listdir', return_value=['photo.jpg']), \
            mock.patch('face_recognition.load_image_file') as mock_load_image_file, \
            mock.patch('face_recognition.face_encodings',
                       return_value=[]):  # Mock face_recognition to return no face encodings
        mock_load_image_file.return_value = 'mock_image'  # Return a mock image object
        result = load_reference_face('test/path')
        assert result is None  # The function should return None if no face is detected in the image
...
def test_face_in_reference_photo():
    ...
    with mock.patch('os.listdir', return_value=['photo.jpg']), \
            mock.patch('face_recognition.load_image_file') as mock_load_image_file, \
            mock.patch('face_recognition.face_encodings',
                       return_value=[mock_encoding]):  # Mock face_recognition to return a face encoding
        mock_load_image_file.return_value = 'mock_image'  # Return a mock image object
        result = load_reference_face('test/path')
        assert result == mock_encoding  # The function should return the face encoding when a face is detected
...
def test_multiple_files_in_folder():
    ...
    with mock.patch('os.listdir', return_value=['photo1.jpg', 'photo2.jpg']), \
            mock.patch('face_recognition.load_image_file') as mock_load_image_file, \
            mock.patch('face_recognition.face_encodings', return_value=[mock_encoding]):
        mock_load_image_file.return_value = 'mock_image'  # Return a mock image object
        result = load_reference_face('test/path')
        assert result == mock_encoding  # It should return the encoding of the first file
def test_no_reference_photo():
    with mock.patch('os.listdir', return_value=[]):  # Mock os.listdir to return an empty list
        result = load_reference_face('test/path')
